document.addEventListener('DOMContentLoaded', function() {
    ...
    // "Speculate" field Placeholder styling
    document.querySelector("#new-text").onfocus = function() {
        document.querySelector("#new-text").placeholder = "";
    }
    document.querySelector("#new-text").onblur = function() {
        document.querySelector("#new-text").placeholder = "Speculate...";
    }
    // Create new post
    document.querySelector("#create-btn").addEventListener("click", function() {
        if (currentUser != null) {
            newText = document.querySelector("#new-text").value;
            if ( newText == "" ) {
                document.querySelector("#create-btn").setAttribute("type", "button");
            } else { document.querySelector("#create-btn").setAttribute("type", "submit"); }
        ...
        } else {      // if not logged in display modal
  ...
  if (event.target == modal) {
    ...
    .then(data => {
        data.forEach(element => {
            ...
            .then(response => response.json())
            .then(likesOfPost => {
                // Get all likes of each post and set up liking button system
...
window.onclick = function(event) {
  if (event.target == modal) {
    ...
    if ( data.length > quantity ) {
        for (let i = (quantity + 1); i < (data.length + 1); i++) {
    ...
    function loadNextPage() {
        ...
        let start = counter;
        if ( (start + quantity - 1) < data.length) {
    ...
    function loadPreviousPage() {
        ...
        let end = start + quantity - 1;
        if ( (end + quantity) > data.length ) {
            for (let i = start + quantity; i < (data.length + 1); i++) {
                document.querySelector(`#p${i-1}`).style.display = 'none';
            }
        }
        else {
